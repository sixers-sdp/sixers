...
def DisplayError(e):
    ...
    if e.code == ErrorCode.EPHIDGET_WRONGDEVICE:
        ...
        sys.stderr.write(
            "\tThis error occurs when you call Phidget functions before a Phidget channel has been opened and "
            "attached.\n "
            "\tTo prevent this error, ensure you are calling the function after the Phidget has been opened and the "
            "program has verified it is attached.")
    elif e.code == ErrorCode.EPHIDGET_NOTCONFIGURED:
        sys.stderr.write(
            "\tThis error code commonly occurs when you call an Enable-type function before all Must-Set Parameters "
            "have been set for the channel.\n "
            "\tCheck the API page for your device to see which parameters are labled \"Must be Set\" on the "
            "right-hand side of the list.")


def DisplayLocatePhidgetsLink():
    print(
        "\n  | In the following example, you will be asked to provide information that specifies which Phidget the "
        "program will use. "
        "\n  | If you are unsure of any of these parameters, be sure to check "
        "www.phidgets.com/docs/Finding_The_Addressing_Information "
